...
const (
	BitsLength           = 64
	Binary               = 2
	DefaultHashKey       = "3b6479d0c6cff7a9"
...
func NewSimpleSimhasher() *Simhasher {
	...
	return NewJiebaSimhasher(jenkinsHasher)
}
...
// 把词典拷贝到项目dict 目录下面
func NewJiebaSimhasher(hasher utils.Hasher) *Simhasher {
	if hasher == nil {
		hasher = utils.NewSipHasher([]byte(DefaultHashKey))
	}
	return &Simhasher{
		extractor: jieba.NewJieba(),
		hasher:    hasher}
}
